...
class OpticalCVFitting(Calculator):
    ...
    def __init__(
        ...
        if measurement is None:
            raise ValueError("OpticalCVFitting requires an ECOpticalMeasurement.")
        super().__init__(
        ...
        self.R_Ohm = R_Ohm
        optical_t_mask = (
            (self.ecoptical.spectrum_series.t >= self.ecoptical.t[0])
            & (self.ecoptical.spectrum_series.t <= self.ecoptical.t[-1])
        )
        Uir_array = (
            self.ecoptical.grab("WE(1).Potential (V)")[1]
            - self.ecoptical.grab("WE(1).Current (A)")[1] * self.R_Ohm
        )
        ...
        ref_index = np.argmin(
            np.abs(self.ecoptical.spectrum_series.t[optical_t_mask] - ref_spectra_t)
        )
        self.truncated_spectra_data_OD = -np.log10(
            self.truncated_spectra_data / self.truncated_spectra_data[ref_index, :]
        )
    ...
    def generate_assigned_spectra(
        ...
        t_mask = (self.truncated_spectra_t >= t_span[0]) & (
            self.truncated_spectra_t <= t_span[1]
        )
        self.assigned_spectra_dict = {}
        for key, value in V_ranges.items():
            if value is not None:
                spectra = (
                    self.truncated_spectra_data_OD[t_mask, :][
                        self.optical_U[t_mask] <= value[1], :
                    ][-1, :]
                    - self.truncated_spectra_data_OD[t_mask, :][
                        self.optical_U[t_mask] >= value[0], :
                    ][0, :]
                )
    ...
    def fit_single_spectrum(
        ...
        U_value,
        fit_results_dict,
        assigned_spectra_dict,
        potential_range_constraints,
    ):
        ...
        if fixed_components_list:
            target_vector = spectrum - np.dot(
                np.array(fixed_weights_list), np.array(fixed_components_list)
            )
        else:
        ...
        result = minimize(
            ...
            initial_weights,
            args=(
                np.array(varying_components_list),
                target_vector,
            ),
            method="COBYLA",
